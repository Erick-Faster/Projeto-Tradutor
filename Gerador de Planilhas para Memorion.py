...
def Reverso(palavras):
    exemplos = [] #Vetor temporario
    exemploFinal = [] #Vetor permanente
    traducoes = []
    traducaoFinal = []
    for i in range (len(palavras)):  #acao para cada palavra
        browser.get("https://context.reverso.net/%C3%BCbersetzung/deutsch-portugiesisch/"+palavras[i]) #site no qual informacao eh extraida
...
def Pons (palavras):
    for i in range (len(palavras)): #Repete de acordo com a qtde de palavras
        browser.get("https://de.pons.com/%C3%BCbersetzung?q="+palavras[i]+"&l=dept&in=&lf=de&qnac=") #Entra no site PONS
        print(palavras[i])
...
palavras = Converte(palavras) #Retira umlauts e eszetts
...
artigos, classes, erros = Pons(palavras) #Elementos que usam o Pons
exemplos, traducoes = Reverso(palavras) #Elementos que usam o Reverso Context
